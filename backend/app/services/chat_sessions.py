...
async def get_session_messages(agent_name: str, session_id: int, limit: int = 200) -> list[MemoryEntry]:
    async with async_session() as db:
        ...
        result = await db.execute(
            select(MemoryEntry)
            .where(MemoryEntry.agent_name == agent_name, MemoryEntry.session_id == session_id)
...
async def refresh_session_metadata(agent_name: str, session_id: int) -> ChatSession:
    async with async_session() as db:
        ...
        user_prompts_result = await db.execute(
            select(MemoryEntry.content)
            .where(
                MemoryEntry.agent_name == agent_name,
                MemoryEntry.session_id == session_id,
                MemoryEntry.role == "user",
            )
            .order_by(MemoryEntry.timestamp.asc(), MemoryEntry.id.asc())
            .limit(MAX_SEED_PROMPTS)
        )
        prompts = list(user_prompts_result.scalars().all())
